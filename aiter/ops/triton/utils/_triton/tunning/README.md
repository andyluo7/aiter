...
To verify that your tunned JSON config files actually is performant and can be correctly picked up by AITER, first you have to copy the generated JSON config files into the config tree. Every family lives in the nested layout `configs/<arch>/<backend>/gemm/<d_type>/` (`<path_to_aiter_root>/aiter/ops/triton/configs/CLAUDE.md` is the authoritative rulebook). Files there carry **no arch prefix** and the default file is named exactly `DEFAULT.json`, so drop the arch prefix when copying:
...
`<d_type>` is the config name lowercased with dashes folded to underscores, so `GEMM-AFP4WFP4_PRESHUFFLED` goes to `gemm_afp4wfp4_preshuffled/`. There is no flat `configs/gemm/` any more — a file copied there is never found.
