...
class MegaMoEV2:
    ...
    # fmt: on
        ...
        # Compact bundles reserve dense source-key payloads so an indexed
        # variant can be selected without rebuilding the operator instance.
        self._s1_payload_dedup = compact
        # fmt: off
    ...
    def _allocate_dispatch_workspace(self, op, metadata_blocks):
        ...
        workspace = {
            ...
            "entry_count": torch.zeros(20 * 16, dtype=torch.int64, device=self.dev),
            "epoch_gate": torch.zeros(20, dtype=torch.int32, device=self.dev),
        ...
        if self.use_kernel_bundle:
            ...
            self._s1_mega(
                *common_args, stream, sort_block_m=config.sort_block_m, tile_n=config.tile_n,
                tile_k=256, num_waves=config.num_waves, grid_mult=config.grid_mult,
                pipe_weights=True, mfma_amajor=config.mfma_amajor,
                swizzle_a=True, async_a_copy=config.async_a_copy,
        ...
        if self.use_kernel_bundle:
            ...
            self._g2_run(
                *common_args, s_fx, BM=stage2.block_m, SBM=config.stage1.sort_block_m,
                BN=stage2.block_n, BK=256, use_nt=stage2.use_nt,
                ...
                skew_cu=stage2.skew_cu, g2_bf16_lds=False, **invariants,
            )
